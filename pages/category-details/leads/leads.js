...
import { merchantDetail, visitRecord, getContacaList, collectAct, collectCancel } from '/api/merchant';
const app = getApp();

Page({
  data: {
    contractorId: '',
    tabList: [{title: '拜访记录'},{title: '联系人'}], // 选项卡名称
  ...
  handleOnTabBarClick(e) {
    ...
    switch (type) {
      case 'cancel': // 取消领取
        const API = isCollect ? collectCancel : collectAct;
        API({contractorId: id})
          .then(res => {
            if (res.data.code === 0) {
          ...
          .catch(err => {
            my.showToast({content: '操作失败', duration: 1000});
          });
        break;
      case 'visit': // 拜访
        if (isCollect) {
      ...
      case 'transform': // 转换
        if (isCollect) {
          my.navigateTo({url: `/pages/merchant/clue-transfer/clue-transfer?id=${id}`});
        } else {
          my.showToast({content: '不是负责人,无法转换', duration: 1000});
        }
        break;
      default:
        break;
