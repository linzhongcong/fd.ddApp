...
import { merchantDetail, visitRecord, getContacaList } from '/api/merchant';
const app = getApp();

const cooperateTypeMap = {
  normal: '正常合作',
  stop: '停止合作',
  silent: '静默',
}
...
Page({
  ...
  goToPage(e) {
    ...
    if (type === 'detail') {
      ...
      my.navigateTo({url: `/pages/category-details/cooperate-facade/cooperate-facade?facadeId=${id}&contractorId=${this.data.contractorId}`});
    }
  },
    ...
    switch (type) {
      case 'visit': // 拜访
        if (isCollect) {
          ...
          my.showToast({content: '仅商务负责人可拜访', duration: 1000});
        }
        break;
      default:
        break;
    ...
    merchantDetail({contractorId})
      .then(res => {
        if (res.data.code === 0) {
          ...
          detailData.merchant_type = formatShop(detailData.merchant_type);
          detailData.cooperateTypeText = cooperateTypeMap[detailData.cooperateType];
